...
function InterestSection({ title, hint, addLabel, items, onRemove, onAddClick }) {
  return (
...
function Mypage() {
  ...
  const listKeyByTab = { category: "categories", region: "regions", storeType: "storeTypes" };
  ...
  const removeItem = (tab, name) => {
    ...
    setState((prev) => ({ ...prev, [key]: prev[key].filter((item) => item !== name) }));
  };

  const addItem = (tab, name) => {
    const key = listKeyByTab[tab];
    setState((prev) => {
      if (prev[key].includes(name) || prev[key].length >= LIMITS[tab]) return prev;
  ...
  const handleSave = async () => {
    ...
    try {
      const updated = await updateMyProfile({
        ...
        interests: { categories, regions, storeTypes },
      });
      login(updated);
      navigate("/");
    } finally {
  ...
  return (
    <div className="container mypage">
      <h1>사용자 정보 수정</h1>
      <p className="mypage__desc">회원님의 정보를 관리하고 관심 정보를 수정할 수 있습니다.</p>
        ...
        <div className="mypage__basic-grid">
          ...
          <div className="mypage__password-field">
            <TextField
              ...
              onChange={handleFieldChange}
            />
            <p className="mypage__field-hint">영문, 숫자, 특수문자를 포함한 8자 이상을 권장합니다.</p>
          </div>
          <TextField
            label="비밀번호 확인"
            id="passwordConfirm"
      ...
      {activeModalTab && (
        <AddItemModal
          ...
          selected={{ category: categories, region: regions, storeType: storeTypes }}
          limits={LIMITS}
          onAdd={addItem}
